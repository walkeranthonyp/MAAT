...
out <- wrapper_object$.test_con(
  mc=F, oconf=F,
  sfnames=
    list(leaf.tcor_asc.vcmax = 'f_tcor_asc_Arrhenius',
...
wrapper_object$init_dynamic
wrapper_object$model$pars$n_pools
...
dim(wrapper_object$dataf$pars_lklihood)
t(wrapper_object$dataf$pars_lklihood)
dim(wrapper_object$dataf$pars_array)
wrapper_object$wpars$mcmc
...
lm(wrapper_object$dataf$obs~as.vector(wrapper_object$dataf$met))


# Canopy
source('wrapper_object.R')
out <- wrapper_object$.test_can(mc=T,verbose=F)
wrapper_object$dataf$met
head(out[[1]])
print(out[[2]])
wrapper_object$model$state
...
# Soil_decomp
source('wrapper_object.R')
out <- wrapper_object$.test_soil(mc=F,verbose=F)
names(wrapper_object)
