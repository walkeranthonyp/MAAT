...
f_canlight_beerslaw_goudriaan <- function(.,l,...) {
  ...
  .$state$vert$apar_sun[] <- .$state_pars$k_dir * .$env$par_dir * exp(-.$state_pars$k_dir*(l-.$pars$k_layer))
}
...
f_canlight_beerslaw <- function(.,l,...){
  ...
  .$state$vert$apar_sun[] <- .$env$par_dir * exp(-.$state_pars$k_dir*(l-.$pars$k_layer))
}
