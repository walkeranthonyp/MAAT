...
leaf_object$.test_aci <- function(., leaf.par=c(100,1000), leaf.ca_conc=seq(0.1,1500,50), 
  ...
  # configure met data and run
  .$dataf      <- list()
  .$dataf$met  <- t(expand.grid(mget(c('leaf.ca_conc','leaf.par'))))      
  .$dataf$lm   <- dim(.$dataf$met)[2] 
  .$dataf$mout <- .$run() 
  .$dataf$out  <- data.frame(do.call(rbind,lapply(1:length(.$dataf$met[,1]),.$run_met)))
  
  print(cbind(t(.$dataf$met),.$dataf$out))
  p1 <- xyplot(A~ca|as.factor(.$dataf$met['leaf.par',]), .$dataf$out, groups=unlist(lim), abline=0,
               ylab=expression('A ['*mu*mol*' '*m^-2*s-1*']'), xlab=expression(C[a]*' [Pa]'),
...
leaf_object$.test_aci_light <- function(., leaf.par=seq(10,2000,50), leaf.ca_conc=seq(1,1200,50),
  ...
  .$dataf$mout     <- .$run() 
  .$dataf$out      <- data.frame(do.call(rbind,lapply(1:length(.$dataf$met[,1]),.$run_met)))
  .$dataf$out_full <- cbind(t(.$dataf$met), .$dataf$out )
  
  p1 <- xyplot(A~ca, .$dataf$out_full, subset=leaf.par==1010, abline=0,groups=unlist(lim),
               ylab=expression('A ['*mu*mol*' '*m^-2*s-1*']'), xlab=expression(C[a]*' [Pa]'),
               panel=function(subscripts=subscripts, ... ) {
                 if(diag) {
