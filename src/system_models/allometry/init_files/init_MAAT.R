################################
#
# MAAT allometry model - initialisation script
# 
...
# This script initialises the allometry version of MAAT
...
init_static <- list(
  fnames = list( allometry = fnames.static),
  pars   = list( allometry = pars.static),
  env    = list( allometry = env.static)
)

init_dynamic <- list(
  fnames    = list( allometry = fnames.var),
  pars      = list( allometry = pars.var),
  pars_proc = list( allometry = pars_proc.var),
  pars_eval = list( allometry = pars_eval.var),
  env       = list( allometry = env.var)
)
