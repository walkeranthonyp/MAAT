...
radar_plot <- function(df1, var1='type', var2=NULL, lnames=NULL, ... ) {
  ...
  # define plot layout
  layout <- if(var2_l[1]==1) c(length(var2_l),length(var1_l)) else c(length(var1_l),length(var2_l))
  if(length(lnames)>1) layout[1] <- layout[1] + 1 
  print(layout)
  par(mfrow=layout,mar=c(0,2,0,0))  
  
  # subsetting loops
  for(v1 in var1_l) {
    df1_sub     <- subset(df1,df1[,var1_c]==v1) 
    
    # reget var2 labels in case there are different numbers of uniques values of va2 within each value of var1
    ...
    for(v2 in var2_l) {
      ...
      # radar plot
      title <- if(is.null(var2)) v1 else paste(v1,v2) 
  ...
  for( l in lnames ) {
    ...
    n <- length(l)
    colors_border <- viridis(n[1])
    legend(x=1, y=9, legend = l, bty = "n", 
           pch=20 , col=colors_border , text.col = "grey", cex=1.5, pt.cex=1)
  }
